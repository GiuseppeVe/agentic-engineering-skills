...
- **REQ-005** [behavior] — README includes supporting-skill usage for `Impeccable`, `UI UX Pro Max`, `Caveman`, and `How to Use Codex`, a conditional end-to-end example, and an explicit note that Orientated E2E Testing v5.2 is planned and excluded. _Acceptance:_ `rg -ni "Impeccable|UI UX Pro Max|Caveman|How to Use Codex|Graph Engineering V5\.2|planned|excluded" README.md` finds each item. _Satisfied by:_ Task 1.
...
- [ ] **Step 5: Add one short conditional example and the Orientated E2E Testing v5.2 status note.** Show the large/uncertain path and state that a small clear change can start with Codex Implement while retaining relevant verification and owner gates.
