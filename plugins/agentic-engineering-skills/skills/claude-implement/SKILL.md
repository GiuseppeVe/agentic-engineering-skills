...
```powershell
claude -p --model fable --effort xhigh "<planner, tester, or reviewer prompt>"
claude -p --model opus --effort high "<one worker packet>"
```

If the host supports per-agent model selection, bind Sol roles to `fable` at `xhigh` and workers to `opus` at `high`. If it does not, keep contexts isolated and report: `Reduced assurance: role isolation verified; Sol/Terra model routing unavailable.` Never claim model routing without evidence.
